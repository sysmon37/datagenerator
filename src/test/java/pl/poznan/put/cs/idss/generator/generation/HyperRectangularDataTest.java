...
public class HyperRectangularDataTest {
    ...
    private static final double borderSize = 4.0;
    ...
    public void whenGenerateCorePointIsCalled_returnsCorePoint() {
        ...
        DataShape shape = new HyperRectangularDataShape(
                new Region(1.0, ShapeType.RECTANGLE, new Coordinate(middleCoordinates), new Size(axisLengths), BorderType.FIXED, borderSize, outlierForbiddenZone, new Distribution(DistributionType.UNIFORM), null),
                generator,
                null);
        shape.getRegion().updateRadiuses();
        assertEquals(new Point(corePointCoordinates), shape.generateSafePoint());
    }
    ...
    public void whenGenerateOverlappingPointIsCalled_returnsOverlappingPoint() {
        ...
        DataShape shape = new HyperRectangularDataShape(
                ...
                generator);
        shape.getRegion().updateRadiuses();
        assertEquals(new Point(overlappingPointCoordinates), shape.generateBorderPoint());
    }
    ...
    public void whenMiddlePointDimensionsDoesNotEqualsToAxisLengths_throws() {
        new HyperRectangularDataShape(
                new Region(1.0, ShapeType.RECTANGLE, new Coordinate(middleCoordinates), new Size(Arrays.asList(0.5)), BorderType.FIXED, 0.0, 0.0, new Distribution(DistributionType.UNIFORM), null),                
                null, null);
    }
    ...
    public void whenMarginIsNegative_throws() {
        new HyperRectangularDataShape(
                new Region(1.0, ShapeType.RECTANGLE, new Coordinate(middleCoordinates), new Size(axisLengths), BorderType.FIXED, -1.0, 0.0, new Distribution(DistributionType.UNIFORM), null),                
                null, null);
    }
    ...
    public void whenIsInOutlierForbiddenZone_returnsCorrectValue() {
        ...
        assertTrue(shape.isInNoOutlierRange(new Point(corePointCoordinates)));
        assertFalse(shape.isInNoOutlierRange(new Point(Arrays.asList(41.0, 250.5))));
        assertTrue(shape.isInNoOutlierRange(new Point(overlappingPointCoordinates)));
        assertTrue(shape.isInNoOutlierRange(new Point(Arrays.asList(40.0, 245.))));
        assertFalse(shape.isInNoOutlierRange(new Point(Arrays.asList(42.0, 245.5))));
    }
}
